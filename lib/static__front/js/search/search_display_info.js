...
!function () {
    ...
    funcs.search_info_display_infoo = function (obj) {
        ...
        if (!search_result.season.hasOwnProperty('none') && search_result.season.length > 0) {

            funcs.display_season_search(search_result.season, container_id, suffix);
...
!function () {
    ...
    funcs.display_season_search = function (show_array, container_id, suffix) {

        let template = ``;
        lib._looper_challenge(show_array, function (element) {

            template += funcs.template_each_season(element, suffix);
