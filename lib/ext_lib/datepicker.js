...
// Converts a date into '1984-03-23' format
function getBarFormat(dt) {
...
function chooseDate(e) {
    ...
    document.getElementById(textbox.dataset.display_id).removeAttribute('carrier');
}
...
function createCalendar(div, month) {
    ...
    while (1) { // Loop for each week
        ...
        for (i = 0; i < 7; i++) { // Loop each day of this week
            ...
            inp.setAttribute('date', getBarFormat(date));
...
function showDatePicker(idOfTextbox, display_id) {
...
//    }
    ...
    if (document.getElementById(display_id).getAttribute('carrier') !== idOfTextbox) {

        document.getElementById(display_id).setAttribute('carrier', idOfTextbox);

        document.getElementById(display_id).innerHTML = ``;

        document.getElementById(display_id).appendChild(div);

    } else {

        document.getElementById(display_id).removeAttribute('carrier');
