...
if (checkist\num_of_data_in_table::num_of_data_in_table("audio__info", "*", ["hash" => $_post_["music_hash"]]) > 0) {
    ...
    if (checkist\num_of_data_in_table::num_of_data_in_table("music_favourite", "*", ["hash" => $_post_["music_hash"], "favour_g" => $_session_['g'], "favour_b" => $_session_['b']]) > 0) {

        checkist\delete_data_in_table::delete_data_in_table("music_favourite", ["hash" => $_post_["music_hash"], "favour_g" => $_session_['g'], "favour_b" => $_session_['b']]);

        new returner\final_returner_json(['message' => ["favourite" => false]]);
    } else {

        checkist\add_data_in_table::add_data_in_table("music_favourite", ["hash" => $_post_["music_hash"], "favour_g" => $_session_['g'], "favour_b" => $_session_['b']]);
